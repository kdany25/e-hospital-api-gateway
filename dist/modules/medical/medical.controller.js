...
let MedicalController = class MedicalController {
    ...
    async getMedicalRecords(res, query) {
        ...
        if (patientId) {
            ...
            if (!medicalRecords.length)
                return res.status(common_1.HttpStatus.NOT_FOUND).json({ error: 'No medical records found' });
    ...
    async assignConsultation(payload, res) {
        ...
        if (error)
            return res.status(common_1.HttpStatus.BAD_REQUEST).json({ error: error.response.data });
        return res.status(common_1.HttpStatus.OK).json({ data });
    }
